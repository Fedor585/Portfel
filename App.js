...
export default function App() {
  ...
  /* ======== API: USD/RUB с цепочкой источников ======== */
  async function fetchUsdRub() {
    let v = null;

    // 1) MOEX — спот (USDRUB_TOM)
    try {
      const moexURL =
      ...
      const r = await fetch(moexURL);
      const j = await r.json();
      const last = Number(j?.marketdata?.data?.[0]?.[0]);
      if (Number.isFinite(last) && last > 0) v = last;
    } catch {}

    // 2) ЦБ РФ — официальный курс (обновляется 1 раз в день)
    if (!v) {
      try {
        const r = await fetch("https://www.cbr-xml-daily.ru/daily_json.js");
        const j = await r.json();
        const cb = Number(j?.Valute?.USD?.Value);
        if (Number.isFinite(cb) && cb > 0) v = cb;
      } catch {}
    }

    // 3) Резерв — exchangerate.host
    if (!v) {
      try {
        const r = await fetch(
        ...
        const j = await r.json();
        const ex = Number(j?.rates?.RUB);
        if (Number.isFinite(ex) && ex > 0) v = ex;
      } catch {}
    }
  ...
  function addItem() {
    ...
    if (!Number.isFinite(a) || a <= 0)
      return Alert.alert("Ошибка", "Количество должно быть > 0");
    if (!Number.isFinite(p) || p <= 0)
      return Alert.alert("Ошибка", "Цена ₽ должна быть > 0");
    ...
    return (
      <View
        style={[
          styles.pill,
          {
            backgroundColor: good ? "#064e3b" : "#5b2330",
  ...
  const MarketRow = ({ sym }) => {
    ...
    const m = market[sym];
    const change =
      period === "24h" ? m?.chg24 : period === "7d" ? m?.chg7d : m?.chg30d;
    ...
    return (
      <View style={styles.row}>
        ...
        {/* RIGHT: USD + % в одной строке, ниже — RUB мелко */}
        <View style={styles.priceBlock}>
          ...
          <Text style={styles.rubNow} numberOfLines={1}>
            {m ? fmtRUB(m.rub) : "—"}
          </Text>
        </View>
      </View>
  ...
  return (
    <View style={{ flex: 1, backgroundColor: "#0B1220" }}>
      ...
      {topPad ? (
        <View style={{ height: topPad, backgroundColor: "#0B1220" }} />
      ) : null}
      ...
      >
        ...
        <View style={styles.nav}>
          ...
          >
            <Text style={[styles.tabText, tab === "market" && styles.tabTextActive]}>
              Цены
            </Text>
          </TouchableOpacity>
          <TouchableOpacity
          ...
          >
            <Text
              style={[styles.tabText, tab === "portfolio" && styles.tabTextActive]}
            >
              Портфель
            </Text>
          </TouchableOpacity>
          ...
          <>
            <View style={styles.segment}>
              {["24h", "7d", "30d"].map((p) => (
                ...
                >
                  <Text
                    style={[styles.segText, period === p && styles.segTextActive]}
                  >
                    {p}
                  </Text>
                </TouchableOpacity>
              ...
              >
                <Text style={[styles.segText, { fontSize: 12 }]}>
                  {autoRefresh ? "Авто 5м ✓" : "Авто 5м ✗"}
                </Text>
              </TouchableOpacity>
            </View>
            ...
            <View style={styles.footer}>
              {formOpen ? (
                <View style={styles.form}>
                  ...
                  <TouchableOpacity
                    style={[styles.addBtn, { backgroundColor: "#475569" }]}
...
const styles = StyleSheet.create({
  ...
  rubNow: { color: "#9CA3AF", fontSize: 12 },
