...
export default function App() {
  ...
  /* ======== MARKET ======== */
  const [period, setPeriod] = useState("24h"); // 24h | 7d | 30d | 1y
  ...
  /* ======== USD/RUB (CoinGecko -> MOEX -> CBR -> EXH) ======== */
  async function getUsdRub() {
    // 1) CoinGecko: USDT→RUB (как на криптобиржах)
    try {
      ...
      console.warn("coingecko usd/rub failed", e);
    }
    // 2) MOEX
    try {
      const u =
        "https://iss.moex.com/iss/engines/currency/markets/selt/boards/CETS/securities/USD000UTSTOM.json?iss.meta=off&iss.only=marketdata&marketdata.columns=LAST";
      const r = await fetchWithTimeout(u);
      const j = await r.json();
      const v = Number(j?.marketdata?.data?.[0]?.[0]);
      if (Number.isFinite(v) && v > 0) return v;
    } catch (e) {
      console.warn("moex usd/rub failed", e);
    }
    // 3) CBR
    try {
      const r = await fetchWithTimeout("https://www.cbr-xml-daily.ru/daily_json.js");
      const j = await r.json();
      const v = Number(j?.Valute?.USD?.Value);
      if (Number.isFinite(v) && v > 0) return v;
    } catch (e) {
      console.warn("cbr usd/rub failed", e);
    }
    // 4) exchangerate.host
    try {
      const r = await fetchWithTimeout(
      ...
      console.warn("exchangerate.host usd/rub failed", e);
    }
    return 90;
  }

  async function fetchUsdRub() {
    const v = await getUsdRub();
    ...
    try {
      const url =
        `https://api.coingecko.com/api/v3/coins/markets?vs_currency=usd&ids=${encodeURIComponent(
          GECKO_IDS
        )}&order=market_cap_desc&per_page=250&page=1&sparkline=false&price_change_percentage=24h,7d,30d,1y`;
      ...
      for (const row of arr) {
        ...
        map[sym] = {
          ...
          chg1y: Number(row.price_change_percentage_1y_in_currency),
        };
      }
      setMarket(map);
  ...
  const PLabel = ({ val }) => {
    ...
    return (
      <View
        style={[
          styles.pill,
          { backgroundColor: good ? "#064e3b" : "#5b2330", borderColor: good ? "#10b981" : "#ef4444" },
        ]}
      >
  ...
  const MarketRow = ({ sym }) => {
    ...
    const m = market[sym];
    const change =
      period === "24h" ? m?.chg24 : period === "7d" ? m?.chg7d : period === "30d" ? m?.chg30d : m?.chg1y;

    return (
      <View style={styles.row}>
        {/* ЛЕВО: иконка + названия */}
        <View style={styles.rowLeft}>
          <Image source={meta.icon} style={styles.iconImg} />
          <View style={{ marginLeft: 10, minWidth: 60 }}>
        ...
        <View style={styles.priceBlock}>
          <Text style={styles.priceNow} numberOfLines={1} adjustsFontSizeToFit minimumFontScale={0.85}>
            {m ? fmtUSD(m.usd) : "—"}
          </Text>
  ...
  const PortfolioRow = ({ item }) => {
    ...
    return (
      <View style={styles.row}>
        <View style={styles.rowLeft}>
          {meta.icon ? <Image source={meta.icon} style={styles.iconImg} /> : <View style={styles.iconImg} />}
        ...
        <View style={styles.priceBlock}>
          <Text style={styles.rowSub} numberOfLines={1}>
            {item.amount} × {fmtRUB(unitRub)}
          </Text>
          <Text style={styles.rowValue} numberOfLines={1}>
            {fmtRUB(unitRub * Number(item.amount || 0))}
          </Text>
      ...
      <KeyboardAvoidingView style={styles.container} behavior={Platform.OS === "ios" ? "padding" : undefined}>
        ...
        <View style={styles.header}>
          ...
          <View style={styles.badgeRow}>
            <View style={styles.usdRubBadge}>
              <Text style={styles.usdRubBadgeText}>USD/RUB</Text>
              <Text style={styles.usdRubBadgeValue}>{fmtRUB(usdRub)}</Text>
            </View>
          </View>
        ...
        {tab === "market" && (
          <>
            ...
            <FlatList
              ...
              refreshControl={<RefreshControl refreshing={refreshing} onRefresh={refreshAll} tintColor="#E5E7EB" />}
...
const styles = StyleSheet.create({
  ...
  usdRubBadgeText: { color: "#86efac", fontWeight: "900" },
  usdRubBadgeValue: { color: "#E5E7EB", fontWeight: "800" },
  ...
  rowLeft: { flexDirection: "row", alignItems: "center", flexShrink: 1, minWidth: 120 },
  iconImg: { width: 36, height: 36, borderRadius: 18, backgroundColor: "#0B1220" },

  centerBlock: { flex: 1, alignItems: "center" },

  priceBlock: { maxWidth: "38%", alignItems: "flex-end" },
  priceNow: { color: "#E5E7EB", fontWeight: "800", fontSize: 16 },
  priceRub: { color: "#9CA3AF", fontSize: 12, marginTop: 2 },
