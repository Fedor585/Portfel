...
export default function App() {
  ...
  async function fetchUsdRub() {
    ...
    try {
      ...
      if (Number.isFinite(v)) { setUsdRub(v); return; }
    } catch {}

    // 2) frankfurter.app (fallback)
    try {
      const r2 = await fetch("https://api.frankfurter.app/latest?from=USD&to=RUB");
      const j2 = await r2.json();
      const v2 = Number(j2?.rates?.RUB);
      if (Number.isFinite(v2)) { setUsdRub(v2); return; }
    } catch {}

    // 3) резерв
    setUsdRub((prev) => prev || 90);
  }
  ...
  async function fetchMarket() {
    ...
    try {
      ...
      // Ключ не обязателен, оставляем заглушку; если лимит — просто подождёт
  ...
  const PLabel = ({ val }) => {
    ...
    return (
      <View style={[
        ...
        <Text style={{ color: good ? "#a7f3d0" : "#fecaca", fontWeight: "800", fontSize: 11 }}>
  ...
  const MarketRow = ({ sym }) => {
    ...
    return (
      <View style={styles.row}>
        ...
        {/* правый блок: верхняя строка USD + % по центру, нижняя — RUB мелким */}
...
const styles = StyleSheet.create({
  ...
  row: {
    marginHorizontal: 16, marginVertical: 8, padding: 12, borderRadius: 16,
  ...
  priceRow: { flexDirection: "row", alignItems: "center", columnGap: 6 },
  priceNow: { color: "#E5E7EB", fontWeight: "800", fontSize: 14 },
  priceRub: { color: "#9CA3AF", fontSize: 11, marginTop: 2 },
