...
export default function App() {
  ...
  async function fetchUsdRub() {
    try {
      ...
      throw new Error("CBR empty");
    } catch {
      // 2) запасной источник
      try {
        const r2 = await fetch("https://api.exchangerate.host/latest?base=USD&symbols=RUB");
        const j2 = await r2.json();
        const v2 = Number(j2?.rates?.RUB);
        if (Number.isFinite(v2) && v2 > 0) setUsdRub(v2);
      } catch {}
    }
  }
  ...
  async function fetchMarket() {
    ...
    try {
      ...
      // без ключа: публичный demo-заголовок пустой
  ...
  const MarketRow = ({ sym }) => {
    ...
    return (
      <View style={styles.row}>
        ...
        {/* Правая часть: две строки цены + бейдж изменения */}
        <View style={styles.priceBlock}>
          {m ? (
            <>
              <Text style={styles.priceNow} numberOfLines={1} adjustsFontSizeToFit minimumFontScale={0.8}>
                {fmtUSD(m.usd)}
              </Text>
