...
export default function App() {
  ...
  async function fetchUsdRub() {
    try {
      ...
      const rate = v / nominal;
      if (Number.isFinite(rate) && rate > 0) {
        setUsdRub(rate); setUsdRubSource("CBR"); return rate;
      }
    } catch {}
    try {
      ...
      if (Number.isFinite(rate) && rate > 0) {
        setUsdRub(rate); setUsdRubSource("ERH"); return rate;
      }
    } catch {}
    try {
      ...
      if (Number.isFinite(rate) && rate > 0) {
        setUsdRub(rate); setUsdRubSource("USDT"); return rate;
      }
    } catch {}
    return undefined;
  }
    ...
    return (
      <View style={styles.row}>
        ...
        <View style={styles.priceBlock}>
          <Text style={styles.priceNow}>{m ? `${fmtUSD(m.usd)} • ${fmtRUB(m.rub)}` : "—"}</Text>
          <PLabel val={change} />
        </View>
      </View>
    );
  ...
  const PortfolioRow = ({ item }) => {
    ...
    return (
      <View style={styles.row}>
        ...
        <View style={styles.priceBlock}>
          <Text style={styles.rowSub}>{item.amount} × {fmtRUB(unitRub)}</Text>
          <Text style={styles.rowValue}>{fmtRUB(unitRub * Number(item.amount || 0))}</Text>
...
const styles = StyleSheet.create({
  ...
  badgeRow: { flexDirection: "row", alignItems:
