...
export default function App() {
  ...
  const [sortKey, setSortKey] = useState("price"); // 'price' | 'name' | 'change'
  ...
  async function fetchUsdRub() {
    try {
      ...
      console.warn("USD/RUB error", e);
      setErrorMsg("Не удалось получить курс USD/RUB");
    }
  }
  ...
  async function fetchMarket() {
    ...
    try {
      ...
      setErrorMsg("Не удалось обновить цены монет");
    } finally {
  ...
  // Отсортированный список монет (для экрана "Цены")
  const sortedSymbols = useMemo(() => {
    const arr = [...SYMBOLS];
    if (sortKey === "name") {
      arr.sort((a, b) => (COIN_META[a].name > COIN_META[b].name ? 1 : -1));
    } else if (sortKey === "price") {
      arr.sort((a, b) => (market[b]?.usd ?? 0) - (market[a]?.usd ?? 0));
    } else if (sortKey === "change") {
      const getChg = (sym) => {
        const m = market[sym];
        return period === "24h" ? m?.chg24 : period === "7d" ? m?.chg7d : m?.chg30d;
        };
      arr.sort((a, b) => (getChg(b) ?? -Infinity) - (getChg(a) ?? -Infinity));
    }
    return arr;
  }, [market, period, sortKey]);
        ...
        {tab === "market" && (
          <>
            ...
            <View style={[styles.segment, { marginTop: 8 }]}>
              <TouchableOpacity
                style={[styles.segBtn, sortKey === "price" && styles.segBtnActive]}
                onPress={() => setSortKey("price")}
              >
                <Text style={[styles.segText, sortKey === "price" && styles.segTextActive]}>Цена USD</Text>
              </TouchableOpacity>
              <TouchableOpacity
                style={[styles.segBtn, sortKey === "change" && styles.segBtnActive]}
                onPress={() => setSortKey("change")}
              >
                <Text style={[styles.segText, sortKey === "change" && styles.segTextActive]}>
                  Изм. {period}
                </Text>
              </TouchableOpacity>
              <TouchableOpacity
                style={[styles.segBtn, sortKey === "name" && styles.segBtnActive]}
                onPress={() => setSortKey("name")}
              >
                <Text style={[styles.segText, sortKey === "name" && styles.segTextActive]}>Название</Text>
              </TouchableOpacity>
            </View>
...
const styles = StyleSheet.create({
  ...
  errBox: {
    ...
    borderRadius: 10,
    backgroundColor: "#5b2330",
    borderWidth: 1,
    borderColor: "#ef4444",
  },
  errText: { color: "#fecaca", fontWeight: "700" },
