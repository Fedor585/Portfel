...
export default function App() {
  ...
  const [market, setMarket] = useState({}); // {SYM: {usd, rub, chg24, chg7d, chg30d}}
  ...
  // CoinGecko: https://api.coingecko.com/api/v3/coins/markets?vs_currency=usd&ids=...&price_change_percentage=24h,7d,30d
  async function fetchMarket() {
    ...
    try {
      ...
      const r = await fetch(url, { headers: { "x-cg-demo-api-key": "" } }); // без ключа
      const arr = await r.json();
      const map = {};
      for (const row of arr) {
        // находим наш символ по id
  ...
  const PortfolioRow = ({ item }) => {
    ...
    return (
      <View style={styles.row}>
        <View style={styles.rowLeft}>
          <View style={[styles.icon, { backgroundColor: meta.color || "#334155" }]} />
          <View style={{ marginLeft: 10 }}>
...
const styles = StyleSheet.create({
  ...
  badgeRow: { flexDirection: "row", gap: 10, alignItems: "center", marginTop: 8 },
  usdtBadge: { flexDirection: "row", alignItems: "center", gap: 8, paddingHorizontal: 10, paddingVertical: 6, backgroundColor: "#14532D", borderRadius: 100 },
  ...
  segment: { flexDirection: "row", gap: 8, paddingHorizontal: 12, marginTop: 10, justifyContent: "center", flexWrap: "wrap" },
  ...
  pill: { marginTop: 4, paddingVertical: 4, paddingHorizontal: 8, borderRadius: 999, borderWidth: 1 },
