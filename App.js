...
export default function App() {
  ...
  const [usdRubSource, setUsdRubSource] = useState("—");
  ...
  async function fetchUsdRub() {
    ...
    try {
      ...
      const nominal = Number(j?.Valute?.USD?.Nominal || 1);
      const rate = v / nominal; // ₽ за 1 USD
      if (Number.isFinite(rate) && rate > 0) {
        setUsdRub(rate); setUsdRubSource("CBR");
        return rate;
      }
    } catch {}
    // 2) exchangerate.host
    try {
      ...
      const j = await r.json();
      const rate = Number(j?.rates?.RUB);
      if (Number.isFinite(rate) && rate > 0) {
        setUsdRub(rate); setUsdRubSource("ERH");
        return rate;
      }
    } catch {}
    // 3) запасной — USDT в ₽ на CoinGecko
    try {
      const r = await fetch("https://api.coingecko.com/api/v3/simple/price?ids=tether&vs_currencies=rub");
      const j = await r.json();
      const rate = Number(j?.tether?.rub);
      if (Number.isFinite(rate) && rate > 0) {
        setUsdRub(rate); setUsdRubSource("USDT");
        return rate;
      }
    } catch {}
    return undefined;
  ...
  /* ======== API: Market ======== */
  async function fetchMarket(rateOverride) {
    const rate = Number.isFinite(rateOverride) ? rateOverride : usdRub;
    ...
    return (
      <View style={styles.row}>
        ...
        <View style={styles.priceBlock}>
          <Text style={styles.priceNow} numberOfLines={1}>{m ? `${fmtUSD(m.usd)} • ${fmtRUB(m.rub)}` : "—"}</Text>
          <PLabel val={change} />
        </View>
      </View>
    );
      ...
      <KeyboardAvoidingView style={styles.container} behavior={Platform.OS === "ios" ? "padding" : undefined}>
        ...
        <View style={styles.header}>
          ...
          <Text style={styles.updateText}>
            Обновлено: {lastMarketUpdate ? lastMarketUpdate.toLocaleTimeString() : "—"} {usdRubSource !== "—" ? `• ${usdRubSource}` : ""}
          </Text>
        </View>
        ...
        {tab === "market" && (
          <>
            ...
            <FlatList
              ...
              refreshControl={
                <RefreshControl refreshing={loadingMarket} onRefresh={refreshAll} tintColor="#fff" titleColor="#fff" />
              }
            />
          </>
        )}
...
const styles = StyleSheet.create({
  ...
  nav: {
    flexDirection: "row", marginTop: 12, marginHorizontal: 12, borderRadius: 14,
  ...
  usdtBadge: {
    ...
    paddingHorizontal: 10, paddingVertical: 6, backgroundColor: "#14532D", borderRadius: 100
  },
